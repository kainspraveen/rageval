...
from text_utils import GRADE_DOCS_PROMPT, GRADE_ANSWER_PROMPT, GRADE_DOCS_PROMPT_FAST, GRADE_ANSWER_PROMPT_FAST, GRADE_ANSWER_PROMPT_BIAS_CHECK, GRADE_ANSWER_PROMPT_STEP, QA_CHAIN_PROMPT
...
def generate_eval(text, chunk, logger):
    ...
    chain = QAGenerationChain.from_llm(
        VertexAI(model_name ="gemini-1.0-pro-001")
    )
    eval_set = []
...
def make_llm(model):
    ...
    #Note TODO(Kains): WIP -> additional DB custom models
...
def make_retriever(splits, retriever_type, embeddings, num_neighbors, llm, logger):
    ...
    if embeddings == "vertex-gecko":
        ...
        PROJECT_ID = ""
        embd = VertexAIEmbeddings(model_name=EMBEDDING_MODEL,
                                        # project=PROJECT_ID,
                                        location=LOCATION, 
                                )
    else:
    ...
    if retriever_type == "similarity-search":
        ...
        vectorstore = Chroma(collection_name = "collection-ie", embedding_function = embd).from_texts(texts = splits, embedding = embd)
        retriever  = vectorstore.as_retriever(k = num_neighbors)
...
def grade_model_answer(predicted_dataset, predictions, grade_answer_prompt, logger):
    ...
    elif grade_answer_prompt == "Step wise Grading":
        prompt = GRADE_ANSWER_PROMPT_STEP
    else:
        prompt = GRADE_ANSWER_PROMPT

    # Note: GPT-4 grader is advised by OAI 
    eval_chain = QAEvalChain.from_llm(llm = VertexAI(model_name ="gemini-1.0-pro-001"),
                                      prompt=prompt)
...
def grade_model_retrieval(gt_dataset, predictions, grade_docs_prompt, logger):
    ...
    #TODO(Kains): 1. interactive GEVAL metric flow; 2. Ragas with Gemini vertex
...
@app.get("/")
async def root():
    return {"message": "Welcome to the DB Interactive Evaluator!"}
